...
int DSCSERIALIZE_JSON_COMPACT_deploy_program_request( deploy_program_request *pst , char *encoding , char *buf , int *p_len )
{
	...
	len=SNPRINTF(buf,remain_len,"\"program\":"); if(len<0||remain_len<len)return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"\""); if(len<0||remain_len<len)return -1; buf+=len; remain_len-=len;
	JSONESCAPE_EXPAND(pst->program,strlen(pst->program),buf,len,remain_len); if(len<0)return -7; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"\""); if(len<0||remain_len<len)return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"}"); if(len<0||remain_len<len)return -1; buf+=len; remain_len-=len;
...
int CallbackOnJsonNode_deploy_program_request( int type , char *jpath , int jpath_len , int jpath_size , char *node , int node_len , char *content , int content_len , void *p )
{
	...
	else if( type & FASTERJSON_NODE_LEAF )
	{
		...
		{JSONUNESCAPE_FOLD(content,content_len,pst->program,len,sizeof(pst->program)-1); if(len<0)return -7;}
	}
