...
int DSCSERIALIZE_JSON_COMPACT_query_workers_response( query_workers_response *pst , char *encoding , char *buf , int *p_len )
{
	...
	len=SNPRINTF(buf,remain_len,","); if( len < 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"\"nodes\":"); if( len <= 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"["); if( len <= 0 ) return -1; buf+=len; remain_len-=len;
			for( index[1] = 0 ; (pst->_nodes_count==0)?(index[1]<3):(index[1]<pst->_nodes_count) ; index[1]++ )
			{
	len=SNPRINTF(buf,remain_len,"{"); if( len <= 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"\"node\":"); if( len <= 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"{"); if( len <= 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"\"ip\":"); if( len < 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"\""); if( len < 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"%s",pst->nodes[index[1]].node.ip); JSONESCAPE_EXPAND(buf,len,remain_len); if( len < 0 ) return -1;
	len=SNPRINTF(buf,remain_len,"\""); if( len < 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,","); if( len < 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"\"port\":"); if( len < 0 ) return -1; buf+=len; remain_len-=len;
	len=SNPRINTF(buf,remain_len,"%d",pst->nodes[index[1]].node.port); JSONESCAPE_EXPAND(buf,len,remain_len); if( len < 0 ) return -1;
	len=SNPRINTF(buf,remain_len,"}"); if( len <= 0 ) return -1; buf+=len; remain_len-=len;
			if((pst->_nodes_count==0)?(index[1]<3-1):(index[1]<pst->_nodes_count-1))
			{ len=SNPRINTF(buf,remain_len,"},"); if( len <= 0 ) return -1; buf+=len; remain_len-=len; }
			else
			{ len=SNPRINTF(buf,remain_len,"}"); if( len <= 0 ) return -1; buf+=len; remain_len-=len; }
		}
...
int CallbackOnJsonNode_query_workers_response( int type , char *jpath , int jpath_len , int jpath_size , char *node , int node_len , char *content , int content_len , void *p )
{
	...
	if( type & FASTERJSON_NODE_BRANCH )
	{
		...
		{if( pst->_nodes_count > 3 ) return -8;}
		}
	...
	else if( type & FASTERJSON_NODE_LEAF )
	{
		...
		{NATOI(content,content_len,pst->count);}
				...
				{JSONUNESCAPE_FOLD(content,content_len,pst->nodes[pst->_nodes_count].node.ip,sizeof(pst->nodes[pst->_nodes_count].node.ip)-1,return -7);}
				/* port */
