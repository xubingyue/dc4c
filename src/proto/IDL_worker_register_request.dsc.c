...
int DSCINIT_worker_register_request( worker_register_request *pst )
{
	int	index[10] = { 0 } ; index[0] = 0 ;
	memset( pst , 0x00 , sizeof(worker_register_request) );
	return 0;
}

#include "fasterjson.h"

int DSCSERIALIZE_JSON_COMPACT_worker_register_request( worker_register_request *pst , char *encoding , char *buf , int *p_len )
{
	int	remain_len ;
	int	len ;
	char	tabs[10+1] ;
...
funcCallbackOnJsonNode CallbackOnJsonNode_worker_register_request ;
int CallbackOnJsonNode_worker_register_request( int type , char *jpath , int jpath_len , int jpath_size , char *node , int node_len , char *content , int content_len , void *p )
{
	worker_register_request	*pst = (worker_register_request*)p ;
	int	index[10] = { 0 } ;
	int	len ;
...
int DSCDESERIALIZE_JSON_COMPACT_worker_register_request( char *encoding , char *buf , int *p_len , worker_register_request *pst )
{
	char	jpath[ 1024 + 1 ] ;
	int	nret = 0 ;
	memset( jpath , 0x00 , sizeof(jpath) );
	nret = TravelJsonBuffer( buf , jpath , sizeof(jpath) , & CallbackOnJsonNode_worker_register_request , (void*)pst ) ;
	if( nret )
		return nret;
