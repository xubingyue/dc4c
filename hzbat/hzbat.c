...
void OnBeginDagBatchProc( struct Dc4cDagSchedule *p_sched , struct Dc4cDagBatch *p_batch , struct Dc4cApiEnv *penv , void *p1 )
{
	...
	nret = DB_pm_hzbat_batches_info_update_for_begin_datetime_and_progress_by_schedule_name_and_batch_name( DC4CGetDagScheduleName(p_sched) , DC4CGetDagBatchName(p_batch) , & batches_info ) ;
	if( nret )
	{
		printf( "DB_pm_hzbat_batches_info_update_for_begin_datetime_and_progress_by_schedule_name_and_batch_name failed[%d]\n" , nret );
...
void OnFinishDagBatchProc( struct Dc4cDagSchedule *p_sched , struct Dc4cDagBatch *p_batch , struct Dc4cApiEnv *penv , void *p1 )
{
	...
	batches_info.progress = DC4CGetDagBatchProgress(p_batch) ;
	nret = DB_pm_hzbat_batches_info_update_for_end_datetime_and_progress_by_schedule_name_and_batch_name( DC4CGetDagScheduleName(p_sched) , DC4CGetDagBatchName(p_batch) , & batches_info ) ;
	if( nret )
	{
		printf( "DB_pm_hzbat_batches_info_update_for_end_datetime_and_progress_by_schedule_name_and_batch_name failed[%d]\n" , nret );
		DB_pm_hzbat_batches_info_debug_print( "ERROR" , & batches_info , __FILE__ , __LINE__ );
		dbRollback();
		return;
	}
...
void OnBeginDagBatchTaskProc( struct Dc4cDagSchedule *p_sched , struct Dc4cDagBatch *p_batch , struct Dc4cApiEnv *penv , int task_index , void *p1 )
{
	...
	nret = DB_pm_hzbat_batches_tasks_update_for_begin_datetime_and_progress_by_schedule_name_and_batch_name_and_order_index( DC4CGetDagScheduleName(p_sched) , DC4CGetDagBatchName(p_batch) , DC4CGetBatchTasksOrderIndex(penv,task_index) , & batches_tasks ) ;
	if( nret )
	{
		printf( "DB_pm_hzbat_batches_tasks_update_for_begin_datetime_and_progress_by_schedule_name_and_batch_name_and_order_index failed[%d]\n" , nret );
...
void OnFinishDagBatchTaskProc( struct Dc4cDagSchedule *p_sched , struct Dc4cDagBatch *p_batch , struct Dc4cApiEnv *penv , int task_index , void *p1 )
{
	...
	if( nret )
	{
		printf( "DB_pm_hzbat_batches_tasks_update_for_end_datetime_and_progress_and_error_and_status_by_schedule_name_and_batch_name_and_order_index failed[%d]\n" , nret );
		DB_pm_hzbat_batches_tasks_debug_print( "ERROR" , & batches_tasks , __FILE__ , __LINE__ );
		dbRollback();
		return;
	}
