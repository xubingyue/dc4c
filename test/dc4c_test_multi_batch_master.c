...
int main( int argc , char *argv[] )
{
	...
	if( argc >= 1 + 4 )
	{
		...
		{
			nret = DC4CInitEnv( & (ppenvs[envs_index]) , argv[1] ) ;
			if( nret )
			{
			...
			DC4CSetOptions( ppenvs[envs_index] , DC4C_OPTIONS_INTERRUPT_BY_APP );
			...
			for( i = 0 , p_task = tasks_array ; i < tasks_count ; i++ , p_task++ )
			{
				if( repeat_task_flag == 1 )
					...
					strcpy( p_task->program_and_params , argv[5+i] );
				p_task->timeout = 0 ;
			}
			
			nret = DC4CBeginBatchTasks( ppenvs[envs_index] , workers_count , tasks_array , tasks_count ) ;
			free( tasks_array );
			if( nret )
			{
		...
		while(1)
		{
			...
			{
				printf( "DC4CPerformMultiBatchTasks ok , penv[%p]\n" , penv );
		...
		for( envs_index = 0 ; envs_index < envs_count ; envs_index++ )
		{
			DC4CCleanEnv( & (ppenvs[envs_index]) );
		}
		free( ppenvs );
