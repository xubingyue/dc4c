...
EXEC SQL BEGIN DECLARE SECTION ;
	...
	char dag_schedule_begin_datetime[ 19 + 1 ] ;	short dag_schedule_begin_datetime_id ;
	char dag_schedule_end_datetime[ 19 + 1 ] ;	short dag_schedule_end_datetime_id ;
	int dag_schedule_progress ;	short dag_schedule_progress_id ;
...
void DSCINITV_dag_schedule()
{
	...
	memset( dag_schedule_begin_datetime , 0x00 , sizeof(dag_schedule_begin_datetime) );
	dag_schedule_begin_datetime_id = 0 ;
	memset( dag_schedule_end_datetime , 0x00 , sizeof(dag_schedule_end_datetime) );
	dag_schedule_end_datetime_id = 0 ;
	dag_schedule_progress = 0 ;
	dag_schedule_progress_id = 0 ;
	return;
}
...
void DSCVTOS_dag_schedule( dag_schedule *pst )
{
	...
	strcpy( pst->begin_datetime , dag_schedule_begin_datetime );
	strcpy( pst->end_datetime , dag_schedule_end_datetime );
	pst->progress = dag_schedule_progress ;
	return;
}
...
void DSCSTOV_dag_schedule( dag_schedule *pst )
{
	...
	strcpy( dag_schedule_begin_datetime , pst->begin_datetime );
	strcpy( dag_schedule_end_datetime , pst->end_datetime );
	dag_schedule_progress = pst->progress ;
	return;
}
...
void DSCSQLACTION_UPDATE_dag_schedule_SET_begin_datetime_progress_WHERE_schedule_name_E( dag_schedule *pst )
{
	DSCSTOV_dag_schedule( pst );
	
	EXEC SQL
		UPDATE	dag_schedule
		SET	begin_datetime = :dag_schedule_begin_datetime, progress = :dag_schedule_progress
		WHERE	schedule_name = :dag_schedule_schedule_name
		;
	return;
}

void DSCSQLACTION_UPDATE_dag_schedule_SET_end_datetime_progress_WHERE_schedule_name_E( dag_schedule *pst )
{
	DSCSTOV_dag_schedule( pst );
	
	EXEC SQL
		UPDATE	dag_schedule
		SET	end_datetime = :dag_schedule_end_datetime, progress = :dag_schedule_progress
		WHERE	schedule_name = :dag_schedule_schedule_name
		;
	return;
}
